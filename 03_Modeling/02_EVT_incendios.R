library(TFMevt)
library(tidyverse)


VAL_incendios_EVT_clusters_clara <- read.csv("data/data_VAL/VAL_incendios_EVT_clusters_clara.csv")

VAL_incendios_EVT_clusters_clara$fecha <- as.Date(VAL_incendios_EVT_clusters_clara$fecha)

VAL_incendios_EVT_clusters_clara <- VAL_incendios_EVT_clusters_clara[,-1]

View(VAL_incendios_EVT_clusters_clara)


# VAL_incendios_EVT_clusters_clara <- VAL_incendios_EVT_clusters_clara %>%
#   mutate(cluster  = ifelse(cluster == 3 &
#                              lat < 39, 2,cluster) )

VAL_incendios_EVT_clusters_clara$cluster <- as.factor(VAL_incendios_EVT_clusters_clara$cluster)

incendios_cluster_1 <- VAL_incendios_EVT_clusters_clara %>%
  filter(cluster == 1)%>%
  group_by(fecha)%>%
  summarise(superficie = max(superficie)) 

incendios_cluster_2 <- VAL_incendios_EVT_clusters_clara %>%
  filter(cluster == 2)%>%
  group_by(fecha)%>%
  summarise(superficie = max(superficie)) 

incendios_cluster_3 <- VAL_incendios_EVT_clusters_clara %>%
  filter(cluster == 3)%>%
  group_by(fecha)%>%
  summarise(superficie = max(superficie)) 

incendios_cluster_4 <- VAL_incendios_EVT_clusters_clara %>%
  filter(cluster == 4)%>%
  group_by(fecha)%>%
  summarise(superficie = max(superficie))


incendios_cluster_1 %>%
  ggplot()+
  geom_line(aes(fecha, superficie), color ="tomato") + 
  theme_minimal()+
  ylab("Superficie quemada")+
  xlab("Fecha")+
  labs(title = "incendios en España",
       subtitle = "Zona 1 - Cordillera Pirenaica/ Baleares")


incendios_cluster_2 %>%
  ggplot()+
  geom_line(aes(fecha, superficie), color ="darkgreen") + 
  theme_minimal()+
  ylab("Superficie quemada")+
  xlab("Fecha")+
  labs(title = "incendios en España",
       subtitle = "Zona 2 - Sistema Bético")


incendios_cluster_3 %>%
  ggplot()+
  geom_line(aes(fecha,superficie), color ="cyan3") + 
  theme_minimal()+
  ylab("superficie quemada")+
  xlab("Fecha")+
  labs(title = "Incendios en España",
       subtitle = "Zona 3 - Macizo Galaico")


incendios_cluster_4 %>%
  ggplot()+
  geom_line(aes(fecha, superficie), color ="purple") +
  theme_minimal()+
  ylab("Superficie quemada")+
  xlab("Fecha")+
  labs(title = "incendios en España",
       subtitle = "Zona 4 - Islas Canarias")





# EVT---------------------------------------------------------------------------

#-------------------------------------------------------------------------------
# cluster_1
quantile(incendios_cluster_1$mag)

fit_cluster_1 <- TFMevt::fitGEV(incendios_cluster_1$mag, c(0.1,0.1,0.1))
fit_cluster_1


TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         5)
TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         10)
TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         25)
TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         50)
TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         100)

TFMevt::return_level_GEV(fit_cluster_1, 
                         incendios_cluster_1$mag,
                         100, 
                         "plot")




TFMevt::mean_excess_GEV(incendios_cluster_1$mag)



threshold <- 3

fit_cluster_1_GP <- TFMevt::fitGPD(incendios_cluster_1$mag, threshold)
fit_cluster_1_GP



TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = threshold,
                         year = 5)
TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = threshold,
                         year = 10)
TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = threshold,
                         year = 25)
TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = threshold,
                         year = 50)
TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = threshold,
                         year = 100)


TFMevt::return_level_GPD(fit_cluster_1_GP,
                         incendios_cluster_1$mag,
                         threshold = 3,
                         year = 100,
                         "plot")



fit_cluster_1_Gumbel <- TFMevt::fitGumbel(incendios_cluster_1$mag)
fit_cluster_1_Gumbel
extRemes::fevd(incendios_cluster_1$mag, type = "Gumbel")




TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 5)

TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 10)

TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 25)

TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 50)

TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 100)


TFMevt::return_level_Gumbel(fit_cluster_1_Gumbel,
                            incendios_cluster_1$mag,
                            year = 100,
                            "plot")


#-------------------------------------------------------------------------------
# cluster_2
max(incendios_cluster_2$mag)
fit_cluster_2 <- TFMevt::fitGEV(incendios_cluster_2$mag, c(0.1,0.1,0.1))
fit_cluster_2


TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         5)
TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         10)
TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         25)
TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         50)
TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         100)

TFMevt::return_level_GEV(fit_cluster_2, 
                         incendios_cluster_2$mag,
                         100, 
                         "plot")




TFMevt::mean_excess_GEV(incendios_cluster_2$mag)



threshold <- 5

fit_cluster_2_GP <- TFMevt::fitGPD(incendios_cluster_2$mag, threshold)
fit_cluster_2_GP



TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = threshold,
                         year = 5)
TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = threshold,
                         year = 10)
TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = threshold,
                         year = 25)
TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = threshold,
                         year = 50)
TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = threshold,
                         year = 100)


TFMevt::return_level_GPD(fit_cluster_2_GP,
                         incendios_cluster_2$mag,
                         threshold = 3,
                         year = 100,
                         "plot")



fit_cluster_2_Gumbel <- TFMevt::fitGumbel(incendios_cluster_2$mag)
fit_cluster_2_Gumbel



TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 5)

TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 10)

TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 25)

TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 50)

TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 100)


TFMevt::return_level_Gumbel(fit_cluster_2_Gumbel,
                            incendios_cluster_2$mag,
                            year = 100,
                            "plot")



#-------------------------------------------------------------------------------
# cluster_3
summary(incendios_cluster_3$mag)
fit_cluster_3 <- TFMevt::fitGEV(incendios_cluster_3$mag, c(0.1,0.1,0.1))
fit_cluster_3


TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         5)
TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         10)
TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         25)
TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         50)
TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         100)

TFMevt::return_level_GEV(fit_cluster_3, 
                         incendios_cluster_3$mag,
                         100*365, 
                         "plot")



fit <- extRemes::fevd(incendios_cluster_3$mag)
extRemes::return.level(fit, return.period = 365*5)
TFMevt::mean_excess_GEV(incendios_cluster_3$mag)



threshold <- 3.2

fit_cluster_3_GP <- TFMevt::fitGPD(incendios_cluster_3$mag, threshold)
fit_cluster_3_GP



TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = threshold,
                         year = 5)
TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = threshold,
                         year = 10)
TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = threshold,
                         year = 25)
TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = threshold,
                         year = 50)
TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = threshold,
                         year = 100)


TFMevt::return_level_GPD(fit_cluster_3_GP,
                         incendios_cluster_3$mag,
                         threshold = 3,
                         year = 100,
                         "plot")



fit_cluster_3_Gumbel <- TFMevt::fitGumbel(incendios_cluster_3$mag)
fit_cluster_3_Gumbel
extRemes::fevd(incendios_cluster_3$mag, type = "Gumbel")




TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 5)

TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 10)

TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 25)

TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 50)

TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 100)


TFMevt::return_level_Gumbel(fit_cluster_3_Gumbel,
                            incendios_cluster_3$mag,
                            year = 100,
                            "plot")





#-------------------------------------------------------------------------------
# cluster_4
max(incendios_cluster_4$mag)
fit_cluster_4 <- TFMevt::fitGEV(incendios_cluster_4$mag, c(0.1,0.1,0.1))
fit_cluster_4


TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         5)
TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         10)
TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         25)
TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         50)
TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         100)

TFMevt::return_level_GEV(fit_cluster_4, 
                         incendios_cluster_4$mag,
                         100*365, 
                         "plot")




TFMevt::mean_excess_GEV(incendios_cluster_4$mag)



threshold <- 3

fit_cluster_4_GP <- TFMevt::fitGPD(incendios_cluster_4$mag, threshold)
fit_cluster_4_GP



TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = threshold,
                         year = 5)
TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = threshold,
                         year = 10)
TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = threshold,
                         year = 25)
TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = threshold,
                         year = 50)
TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = threshold,
                         year = 100)


TFMevt::return_level_GPD(fit_cluster_4_GP,
                         incendios_cluster_4$mag,
                         threshold = 3,
                         year = 100,
                         "plot")



fit_cluster_4_Gumbel <- TFMevt::fitGumbel(incendios_cluster_4$mag)
fit_cluster_4_Gumbel
extRemes::fevd(incendios_cluster_4$mag, type = "Gumbel")




TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 5)

TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 10)

TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 25)

TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 50)

TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 100)


TFMevt::return_level_Gumbel(fit_cluster_4_Gumbel,
                            incendios_cluster_4$mag,
                            year = 100,
                            "plot")


