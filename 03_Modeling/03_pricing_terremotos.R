...
colnames(iris)
set.seed(4595)
data_split <- initial_split(scale(iris[,-5]), strata = "Sepal.Length", prop = 0.75)
...
train_data <- recipe(Sepal.Length~., data = train_data) %>% 
  prep() %>% 
  juice()


k_folds_data <- vfold_cv(train_data, strata = Sepal.Length,v = 10)


hash_rec <- recipe(Sepal.Length~., data = train_data)
